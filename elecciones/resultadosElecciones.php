...
$E_RESULTADOS = $conn_web->prepare("SELECT * FROM `E_RESULTADOS_SECCION`;");
$E_RESULTADOS->execute();
$E_RESULTADOS_RES = $E_RESULTADOS->fetchAll(PDO::FETCH_ASSOC);

if (isset($_GET["seccion"])) {
  $RESULT_ID = isset($_GET["seccion"]) ? $_GET['seccion'] : "";

  $E_RESULTADOS = $conn_web->prepare("SELECT * FROM `E_RESULTADOS` WHERE ID_SECCION = :seccion ;");
  $E_RESULTADOS->bindParam(":seccion", $RESULT_ID);
  $E_RESULTADOS->execute();
  $E_RESULTADOS_S_RES = $E_RESULTADOS->fetchall(PDO::FETCH_ASSOC);
}
        ...
        <div class="row g-2 d-flex align-items-center">
          ...
          <div class="col-lg-10 col-md-4 custom-select">
            <select id="seccion" name="seccion" class="form-select text-center">
              ...
              <?php foreach ($E_RESULTADOS_RES as $value) { ?>
                <option <?php echo ($RESULT_ID == $value["ID"]) ? "selected" : ""; ?> 
                value="<?php echo $value["ID"] ?>"><?php echo $value["SECCION"] ?></option>
              <?php } ?>
            </select>
      ...
      <div class="col-lg-12 d-flex">
        <div class=" container icon-boxes d-flex flex-column justify-content-center">
          <div class="row justify-content-center">
            ...
            <?php foreach ($E_RESULTADOS_S_RES as $key => $value) { ?>
