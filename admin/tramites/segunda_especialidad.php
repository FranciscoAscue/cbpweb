...
$sentencia = $conn_web->prepare("SELECT * FROM `SEGUNDA_ESPECIALIDAD`;");
$sentencia->execute();
...
if (isset($_GET['ID'])) {
  ...
  $sentencia = $conn_web->prepare("SELECT PDF FROM `SEGUNDA_ESPECIALIDAD` WHERE ID=:id;");
  ...
  $sentencia1 = $conn_web->prepare("DELETE FROM `SEGUNDA_ESPECIALIDAD` WHERE ID = :id;");
  $sentencia1->bindParam(":id", $ID);
  $sentencia1->execute();
  $mensaje = "Seccion eliminada!";
  header("Location:segunda_especialidad.php?mensaje=" . $mensaje);
  ...
  <ul class="sidebar-nav" id="sidebar-nav">
    ...
    <li class="nav-item">
      ...
      <ul id="tramites" class="nav-content" data-bs-parent="#sidebar-nav">
        ...
        <li>
          <a href="<?php echo $url_base; ?>tramites/segunda_especialidad.php" class="active">
    ...
    <script>
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location = "segunda_especialidad.php";
            }
        })
    </script>
<?php } ?>
  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Segunda Especialidad</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">Tramite de Segunda Especialidad &emsp;
              <a href="segunda_especialidad_agregar.php" class="btn btn-primary">+ Agregar Seccion</a>
            </h5>
                ...
                <tbody>
                  <?php foreach ($busqueda as $registro) { ?>
                    <tr>
                      ...
                      <td scope="row">
                        <a name="" id="" class="btn btn-warning" href="segunda_especialidad_editar.php?ID=<?php echo $registro['ID']; ?>" role="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar"><i class="bi bi-pencil me-1"></i></a>
                        <?php if ($registro["ID"] > 2) { ?>
...
<script>
    function borrar(id) {
        ...
        Swal.fire({
            ...
            /* Read more about isConfirmed, isDenied below */
            if (result.isConfirmed) {
                window.location = "segunda_especialidad.php?ID=" + encodeURI(id);
            }
        })
    }
</script>
