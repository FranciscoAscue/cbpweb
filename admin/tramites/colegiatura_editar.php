...
if (isset($_GET['ID'])) {
  ...
  $NOSOTROS = $conn_web->prepare("SELECT * FROM `COLEGIATURA` WHERE `ID` = :id ;");
...
if(isset($_POST["seccion"])){
  ...
  $sentencia = $conn_web -> prepare("UPDATE `COLEGIATURA` SET `TAG` = :tag ,`SECCION` = :seccion, 
  ...
  if($tmp_pdf!=""){
    ...
    $sentencia1 = $conn_web -> prepare("UPDATE `COLEGIATURA` SET `PDF` = :pdf WHERE ID = :id;");
