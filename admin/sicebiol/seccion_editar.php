...
if (isset($_GET['ID'])) {
  ...
  $NOSOTROS = $conn_web->prepare("SELECT * FROM `SICEBIOL` WHERE `ID` = :id ;");
...
if(isset($_POST["seccion"])){
  ...
  $sentencia = $conn_web -> prepare("UPDATE `SICEBIOL` SET `TAG` = :tag ,`SECCION` = :seccion, 
  ...
  if($tmp_pdf!=""){
    move_uploaded_file($tmp_pdf,"../../sicebiol/docs/".$nombre_archivo_pdf);
    $sentencia = $conn_web->prepare("SELECT PDF FROM `SICEBIOL` WHERE ID=:id;");
    $sentencia->bindParam(":id", $ID);
    $sentencia->execute();
    $lista_files_name = $sentencia->fetch(PDO::FETCH_LAZY);
    if(isset($lista_files_name["PDF"]) && $lista_files_name["PDF"]!=""){
      if(file_exists("../../sicebiol/docs/".$lista_files_name["PDF"])){
          unlink("../../sicebiol/docs/".$lista_files_name["PDF"]);
      }
    }

    $sentencia1 = $conn_web -> prepare("UPDATE `SICEBIOL` SET `PDF` = :pdf WHERE ID = :id;");
...
<aside id="sidebar" class="sidebar">
  ...
  <ul class="sidebar-nav" id="sidebar-nav">
    ...
    <li class="nav-item">
      <a class="nav-link show" data-bs-target="#sicebiol" data-bs-toggle="collapse" href="#">
        <i class="bi bi-file-earmark-person-fill"></i><span>Sicebiol</span><i class="bi bi-chevron-down ms-auto"></i>
      </a>
      <ul id="sicebiol" class="nav-content" data-bs-parent="#sidebar-nav">
        <li>
          <a href="<?php echo $url_base; ?>sicebiol/" class="active">
  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">Sicebiol</li>
            ...
            <form action="" method="post" id="registro" enctype="multipart/form-data">
              ...
              <div class="row mb-3">
                <label for="cv" class="col-sm-3 col-form-label">
                  <a href="/sicebiol/docs/<?php echo $NOSOTROS_RES["PDF"]?>" >ACTUAL PDF</a>
                </label>
                <div class="col-sm-7">
              ...
              <div class="row mb-3 d-flex justify-content-center">
                ...
                <div class="col-sm-2">
                  <a name="" id="" class="btn btn-danger" href="/admin/sicebiol/index.php" role="button">Cancelar</a>
                </div>
              </div>
