...
if (isset($_POST["seccion"])) {
  ...
  $sentencia = $conn_web->prepare("INSERT INTO `E_RESULTADOS_SECCION`(`ID`,`TAG`,`SECCION`) 
...
if (isset($_POST["ed_seccion"])) {
  ...
  $sentencia = $conn_web->prepare("UPDATE `E_RESULTADOS_SECCION` SET TAG = :tag, SECCION = :seccion WHERE ID=:id;");
...
if (isset($_GET['ID_E'])) {
  $ID_E = (isset($_GET['ID_E']) ? $_GET['ID_E'] : "");
  $editar = $conn_web->prepare("SELECT * FROM `E_RESULTADOS_SECCION` WHERE ID = :id;");
...
if (isset($_GET['ID'])) {
  ...
  $sentencia1 = $conn_web->prepare("DELETE FROM `E_RESULTADOS_SECCION` WHERE ID = :id;");
          ...
          <div class="card-body">
            ...
            <form action="" method="post" id="registro" enctype="multipart/form-data">
              ...
              <div class="row mb-3 d-flex justify-content-center">
                ...
                <div class="col-sm-2">
                  <?php if (isset($ID_E)) { ?>
                    <a name="" id="" class="btn btn-danger" href="/admin/elecciones/ResultadoElecciones_seccion.php" role="button">Cancelar</a>
                  <?php } else { ?>
                    <a name="" id="" class="btn btn-danger" href="/admin/elecciones/ResultadoElecciones.php" role="button">Regresar</a>
                  <?php } ?>
                  ...
                  <?php foreach ($busqueda as $registro) { ?>
                    <tr>
                      ...
                      <td scope="row">
                        ...
                        <a name="" id="" class="btn btn-warning" href="ResultadoElecciones_seccion.php?ID_E=<?php echo $registro['ID']; ?>" role="button" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar"><i class="bi bi-pencil me-1"></i></a>
...
<script>
  function borrar(id) {
    ...
    Swal.fire({
      ...
      /* Read more about isConfirmed, isDenied below */
      if (result.isConfirmed) {
        window.location = "ResultadoElecciones_seccion.php?ID=" + encodeURI(id);
      }
    })
  }
</script>
