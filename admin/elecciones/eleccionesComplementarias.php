...
$sentencia = $conn_web->prepare("SELECT *,(SELECT SECCION FROM `E_COMPLEMENTARIAS_SECCION` 
WHERE `E_COMPLEMENTARIAS_SECCION`.`ID` = `E_COMPLEMENTARIAS`.`ID_SECCION` LIMIT 1) AS SECCION FROM `E_COMPLEMENTARIAS`;");
$sentencia->execute();
...
if (isset($_GET['ID'])) {
  ...
  $sentencia1 = $conn_web->prepare("DELETE FROM `E_COMPLEMENTARIAS` WHERE ID = :id;");
    ...
    <script>
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location = "eleccionesComplementarias.php";
            }
        })
    </script>
<?php } ?>
  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">Elecciones Complementarias  &emsp;
            <a href="eleccionesComplementarias_agregar.php" class="btn btn-primary">+ Agregar Documento</a>
            <a href="eleccionesComplementarias_seccion.php" class="btn btn-warning">+ Crear/Editar Seccion</a>
            </h5>
                  ...
                  <?php foreach ($busqueda as $registro) { ?>
                    <tr>
                      ...
                      <a name="" id="" class="btn btn-warning" 
                        href="eleccionesComplementarias_editar.php?ID=<?php echo $registro['ID']; ?>" 
...
<script>
    function borrar(id) {
        ...
        Swal.fire({
            ...
            /* Read more about isConfirmed, isDenied below */
            if (result.isConfirmed) {
                window.location = "eleccionesComplementarias.php?ID=" + encodeURI(id);
            }
        })
    }
</script>
