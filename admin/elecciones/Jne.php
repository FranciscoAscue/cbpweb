...
$sentencia = $conn_web->prepare("SELECT *,(SELECT SECCION FROM `JNE_SECCION` 
WHERE `JNE_SECCION`.`ID` = `JNE`.`ID_SECCION` LIMIT 1) AS SECCION FROM `JNE`;");
...
if (isset($_GET['ID'])) {
  ...
  $sentencia1 = $conn_web->prepare("DELETE FROM `JNE` WHERE ID = :id;");
  ...
  <div class="pagetitle">
    ...
    <nav>
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Jurado Nacional de Elecciones</li>
      </ol>
    </nav>
  </div><!-- End Page Title -->
  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">Jurado Nacional de Elecciones  &emsp;
            <a href="Jne_agregar.php" class="btn btn-primary">+ Agregar Documento</a>
            <a href="Jne_seccion.php" class="btn btn-warning">+ Crear/Editar Seccion</a>
            </h5>
                  ...
                  <?php foreach ($busqueda as $registro) { ?>
                    <tr>
                      ...
                      <a name="" id="" class="btn btn-warning" 
                        href="Jne_editar.php?ID=<?php echo $registro['ID']; ?>" 
...
<script>
    function borrar(id) {
        ...
        Swal.fire({
            ...
            /* Read more about isConfirmed, isDenied below */
            if (result.isConfirmed) {
                window.location = "Jne.php?ID=" + encodeURI(id);
            }
        })
    }
</script>
