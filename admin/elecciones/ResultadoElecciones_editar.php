...
if (isset($_GET['ID'])) {
  ...
  $E_RESULTADOS = $conn_web->prepare("SELECT * FROM `E_RESULTADOS` WHERE `ID` = :id ;");
  $E_RESULTADOS->bindParam(":id", $ID);
  $E_RESULTADOS->execute();
  $E_RESULTADOS_RES = $E_RESULTADOS->fetch(PDO::FETCH_LAZY);
...
if (isset($_POST["titulo"])) {
  ...
  $sentencia = $conn_web->prepare("UPDATE `E_RESULTADOS` SET `TITULO` = :titulo, `ID_SECCION` = :id_seccion , 
  ...
  if ($tmp_foto != "") {
    ...
    $sentencia = $conn_web->prepare("SELECT FOTO FROM `E_RESULTADOS` WHERE ID=:id;");
    $sentencia->bindParam(":id", $ID );
    $sentencia->execute();
    ...
    $sentencia1 = $conn_web->prepare("UPDATE `E_RESULTADOS` SET `FOTO` = :foto WHERE ID = :id;");
  ...
  if ($tmp_pdf != "") {
    ...
    $sentencia = $conn_web->prepare("SELECT PDF FROM `E_RESULTADOS` WHERE ID=:id;");
    $sentencia->bindParam(":id", $ID );
    $sentencia->execute();
    $lista_files_name = $sentencia->fetch(PDO::FETCH_LAZY);
    ...
    $sentencia1 = $conn_web->prepare("UPDATE `E_RESULTADOS` SET `PDF` = :pdf WHERE ID = :id;");
          ...
          <div class="card-body">
            ...
            <form action="" method="post" id="registro" enctype="multipart/form-data">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-7">
                  <input type="text" class="form-control" value="<?php echo $E_RESULTADOS_RES["TITULO"]?>" id="titulo" name="titulo" required>
                </div>
              </div>
              <div class="row mb-3">
                ...
                <div class="col-sm-2">
                <img width="50" src="../../elecciones/docs/<?php echo $E_RESULTADOS_RES["FOTO"]; ?>">
                </div>
              </div>
              <div class="row mb-3">
                ...
                <div class="col-sm-2">
                <a href="../../elecciones/docs/<?php echo $E_RESULTADOS_RES["PDF"]; ?>">PDF</a>
                </div>
              </div>
              <br>
              <div class="row mb-3">
                ...
                <div class="col-lg-5 col-md-4">
                  ...
                  <select id="id_seccion" name="id_seccion" class="form-select  text-center placeholder-center">
                    <?php foreach ($busqueda as $value) { ?>
                      <option <?php echo ($E_RESULTADOS_RES["ID_SECCION"] == $value["ID"]) ? "selected" : ""; ?> value="<?php echo $value["ID"] ?>"><?php echo $value["SECCION"] ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-5">
                  <input type="date" class="form-control" id="fecha" value="<?php echo $E_RESULTADOS_RES["FECHA"]?>" name="fecha" required>
                </div>
              </div>
