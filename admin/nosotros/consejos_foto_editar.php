...
if (isset($_GET['ID'])) {
  ...
  $CONSEJO_F = $conn_web->prepare("SELECT * FROM `CONSEJO_GRUPAL` WHERE `ID` = :id ;");
  $CONSEJO_F->bindParam(":id", $ID);
  $CONSEJO_F->execute();
  $CONSEJO_F_RES = $CONSEJO_F->fetch(PDO::FETCH_LAZY);
...
if(isset($_POST["id_consejo"])){
  ...
  $sentencia = $conn_web -> prepare("UPDATE `CONSEJO_GRUPAL` SET `DESCRIPCION` = :contenido WHERE ID = :id;");
  ...
  if($tmp_foto!=""){
    ...
    $sentencia = $conn_web->prepare("SELECT FOTO FROM `CONSEJO_GRUPAL` WHERE ID=:id;");
    $sentencia->bindParam(":id", $id_consejo);
    $sentencia->execute();
    ...
    $sentencia1 = $conn_web -> prepare("UPDATE `CONSEJO_GRUPAL` SET `FOTO` = :foto WHERE ID = :id;");
  ...
  header("Location:consejos_foto.php?mensaje=".$mensaje);
}
          ...
          <div class="card-body">
            ...
            <form action="" method="post" id="registro" enctype="multipart/form-data">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-2">
                  <img width="50" src="../../assets/img/consejo/<?php echo $CONSEJO_F_RES["FOTO"] ?>">
                </div>
              </div>
              <br>
              <div class="row mb-3">
                ...
                <div class="col-sm-9">
                  <textarea type="" class="form-control" name="contenido" id="contenido"><?php echo $CONSEJO_F_RES["DESCRIPCION"]?></textarea>
              ...
              <div class="row mb-3 d-flex justify-content-center">
                ...
                <div class="col-sm-2">
                  <a name="" id="" class="btn btn-danger" href="/admin/nosotros/consejos_foto.php" role="button">Cancelar</a>
                </div>
              </div>
